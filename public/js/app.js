...
/***/ (() => {
        ...
        var bodyArea = document.getElementById('body-container');
        ...
        document.getElementById("updateForm").addEventListener("submit", function (event) {
          ...
          var formData = new FormData(form); // フォームのデータを取得

          // 追加情報をフォームデータに追加
          var lyrics = editable.innerHTML; //うまく取得してキーにlyricsを設定すればいいんじゃね
          ...
          var decodedHTML = decodeHTMLEntities(encodedHTML);
        ...
        function edit(range, addAttribute) {
          ...
          //ノードが複数のノードに跨っている時
            ...
            var rangeChildNodes = range.extractContents().childNodes;
            console.log("複数ノードです");
            if (hasTextNodes(rangeChildNodes)) {
              ...
              if (getNodesAttributeStatus(addAttribute, rangeChildNodes)) {
                ...
                for (var i = 0, j = rangeChildNodes.length; i < j; i++) {
                  removeAttribute(addAttribute, rangeChildNodes[i]);
                  if (getStyleCount(rangeChildNodes[i]) === 0) {
                    var textContent = document.createTextNode(rangeChildNodes[i].textContent);
        ...
        function getStyleCount(node) {
          var attributes = AttributeManager.getElementAttribute(node);
        ...
        function duplicationJudgment(addAttribute, node) {
          var elementAttribute = AttributeManager.getElementAttribute(node);
        ...
        function removeSpanElement(addAttribute, range) {
          ...
          var middleTextNode = document.createTextNode(range.toString());
          var rangeNumberArray = getNumbersBetween(range.startOffset, range.endOffset);
          var parentElementTextArray = parentElement.textContent.split("");
          ...
          var fragment = stripAttributeFromTag(beforeText, middleTextNode, afterText, addAttribute);
        ...
        function addAttributeToSpanTag(addAttribute, range) {
          ...
          beforeText = nodeSplit(resultNode)[0];
          afterText = nodeSplit(resultNode)[1];
          var parentElementAttributes = AttributeManager.getElementAttribute(parentElement);
          var originalAttribute = [];
          parentElementAttributes.forEach(function (attribute) {
            if (attribute.getValue() !== "") {
        ...
        function combineAdjacentTextNodes(element) {
          ...
          var previousNode;
          for (var i = 0, j = elementChilds.length; i < j; i++) {
            if (elementChilds[i].nodeType === Node.TEXT_NODE && elementChilds[i].textContent !== "" && previousNode !== undefined) {
              if (previousNode.nodeType === Node.TEXT_NODE) {
                var textNode = document.createTextNode(previousNode.textContent + elementChilds[i].textContent);
        ...
        function getAttributeStatus(addAttribute, node) {
          var attribute;
          var status = AttributeManager.getElementAttribute(node);
        ...
        function getNodesAttributeStatus(addAttribute, nodes) {
          for (var i = 0; i < nodes.length; i++) {
            var attributes = AttributeManager.getElementAttribute(nodes[i]);
        ...
        function stripAttributeFromTag(beforeText, middleTextNode, afterText, addAttribute) {
          var beforeNodeContainer = document.createElement("span");
          var afterNodeContainer = document.createElement("span");
          var fragment = document.createDocumentFragment();
          if (beforeText !== "") {
            var beforeChangeTextNode = document.createTextNode(beforeText);
          ...
          if (afterText !== "") {
            var afterChangeTextNode = document.createTextNode(afterText);
        ...
        function removeAttributesFromPartialText(addAttribute, originalAttribute, beforeText, range, afterText) {
          ...
          if (beforeText !== "") {
            var beforeNodeContainer = document.createElement("span");
            var beforeChangeTextNode = document.createTextNode(beforeText);
            for (var i = 0; i < originalAttribute.length; i++) {
              setAttribute(originalAttribute[i], beforeNodeContainer);
            }
            beforeNodeContainer.appendChild(beforeChangeTextNode);
            fragment.appendChild(beforeNodeContainer);
          }
          for (var _i = 0; _i < originalAttribute.length; _i++) {
            setAttribute(originalAttribute[_i], middleSpanContainer);
          }
          ...
          if (afterText !== "") {
            var afterNodeContainer = document.createElement("span");
            var afterChangeTextNode = document.createTextNode(afterText);
            for (var _i2 = 0; _i2 < originalAttribute.length; _i2++) {
              setAttribute(originalAttribute[_i2], afterNodeContainer);
            }
            afterNodeContainer.appendChild(afterChangeTextNode);
            fragment.appendChild(afterNodeContainer);
          }
        ...
        function applyTagToPartialText(addAttribute, originalAttribute, beforeText, range, afterText) {
          ...
          for (var _i3 = 0; _i3 < originalAttribute.length; _i3++) {
            setAttribute(originalAttribute[_i3], middleSpanContainer);
          }
          ...
          if (afterText !== "") {
            var afterNodeContainer = document.createElement("span");
            var afterChangeTextNode = document.createTextNode(afterText);
            for (var _i4 = 0; _i4 < originalAttribute.length; _i4++) {
              setAttribute(originalAttribute[_i4], afterNodeContainer);
            }
            afterNodeContainer.appendChild(afterChangeTextNode);
            fragment.appendChild(afterNodeContainer);
          }
        ...
        function applyAttributesToMultipleNodes(addAttribute, nodes, range) {
          ...
          for (var i = 0, j = nodes.length; i < j; i++) {
            if (nodes[i].nodeName === "SPAN" && !getAttributeStatus(addAttribute, nodes[i])) {
              ...
              var rangeChildNodesContainer = document.createElement("span");
        ...
        function nodeSplit(node) {
          ...
          //文字列"?"後のインデックス番号取得
          for (var _i5 = 0; _i5 < node.length; _i5++) {
            if (node[_i5].indexOf("?") !== -1) {
              afterNodeNumber++;
            }
          }

          //"？"終わりからの文字取得
          for (var _i6 = afterNodeNumber; _i6 < node.length; _i6++) {
            afterText += node[_i6];
          }
        ...
        function emptySpanRemove(node) {
          ...
          for (var i = 0, j = nodeChildNodes.length; i < j; i++) {
        ...
        var ElementAttribute = /*#__PURE__*/function () {
          function ElementAttribute(name, attributeName, value) {
            _classCallCheck(this, ElementAttribute);
            this.name = name;
            this.attributeName = attributeName;
            this.value = value;
          }
          _createClass(ElementAttribute, [{
            key: "getName",
